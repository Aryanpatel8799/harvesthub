...
const registerFarmer = async (req, res) => {
  try {
    ...
    if (farmerExists) {
      return res
        .status(400)
        .json({ message: "Farmer already exists with this email" });
    }
  ...
  try {
    ...
    if (req.files && req.files.farmImages) {
      const imageCaptions = Array.isArray(req.body.imageCaptions)
        ...
        : [];
      const farmImages = req.files.farmImages.map((file, index) => ({
        url: `${baseUrl}/uploads/${file.filename}`,
        caption: imageCaptions[index] || "",
      }));

      // Merge with existing images if any
      const existingImages = existingFarmer.farmImages || [];
      updates.farmImages = [...existingImages, ...farmImages];
    }
    ...
    if (req.files && req.files.farmVideos) {
      const videoCaptions = Array.isArray(req.body.videoCaptions)
        ...
        : [];
      const farmVideos = req.files.farmVideos.map((file, index) => ({
        url: `${baseUrl}/uploads/${file.filename}`,
        caption: videoCaptions[index] || "",
      }));

      // Merge with existing videos if any
      const existingVideos = existingFarmer.farmVideos || [];
      updates.farmVideos = [...existingVideos, ...farmVideos];
    }
