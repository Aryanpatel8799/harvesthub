...
const AIChat = () => {
  ...
  return (
    <div className="container w-full mx-auto px-2">
      <div className="flex flex-col h-[calc(100vh-4rem)] bg-white rounded-lg shadow-lg border border-gray-100">
        {/* Chat Header */}
        <div className="flex items-center gap-4 p-4 border-b bg-gradient-to-r from-harvest-50 to-white">
          <div className={`p-2 rounded-full ${
            user?.type === 'farmer' ? 'bg-harvest-100' : 'bg-orange-100'
          }`}>
            {user?.type === 'farmer' ? (
              <Bot className="h-6 w-6 text-harvest-600" />
            ) : (
              ...
              <div className={`flex gap-3 max-w-[90%] ${message.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                <div className={`w-9 h-9 rounded-full flex items-center justify-center flex-shrink-0 shadow-sm ${
                  ...
                  {message.role === 'user' ? (
                    ...
                    user?.type === 'farmer' ? 
                      <Bot className="h-5 w-5 text-harvest-600" /> : 
            ...
            <div className="flex justify-start">
              <div className="flex gap-3 max-w-[90%]">
                ...
                <div className="bg-white rounded-2xl px-4 py-2.5 shadow-sm border border-gray-100">
                  <Loader2 className="h-5 w-5 animate-spin text-harvest-600" />
                </div>
              </div>
            </div>
          )}
          ...
          <div className="flex flex-wrap gap-2">
            {quickQuestions.map((question, index) => (
              <button
                ...
                disabled={isLoading}
                className="px-3 py-1.5 text-sm bg-gray-50 border border-gray-200 rounded-full text-gray-700 hover:bg-gray-100 flex items-center gap-2 transition-all duration-200 disabled:opacity-50 hover:shadow-sm"
              >
                {user?.type === 'farmer' ? (
                  <Leaf className="h-3.5 w-3.5 text-harvest-600" />
                ) : (
        ...
        <form onSubmit={handleSendMessage} className="p-3 border-t bg-white">
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 px-4 py-2.5 rounded-lg border border-gray-200 focus:border-harvest-600 focus:ring-2 focus:ring-harvest-100 focus:ring-opacity-50 placeholder-gray-400 text-gray-800 transition-all duration-200 text-[15px]"
              disabled={isLoading}
            />
            <button
              type="submit"
              disabled={isLoading || !input.trim()}
              className="bg-harvest-600 text-white px-4 py-2.5 rounded-lg hover:bg-harvest-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 flex items-center justify-center shadow-sm hover:shadow-md"
            >
