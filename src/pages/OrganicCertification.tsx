    ...
    <div className="container mx-auto px-4 py-8">
      ...
      {loading && (
        <div className="text-center py-10">
          <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-harvest-600 mx-auto"></div>
      ...
      {!loading && reports.length === 0 && (
        <div className="text-center py-10 bg-white rounded-lg shadow-sm border border-gray-100 p-6">
          ...
          <p className="text-gray-600 mb-6">You haven't submitted any soil reports for certification</p>
          <button 
            onClick={() => setShowUploadModal(true)}
            className="bg-harvest-600 text-white px-6 py-2 rounded-lg hover:bg-harvest-700 transition-colors"
          >
            ...
            <div key={report._id} className="bg-white rounded-lg shadow-sm border border-gray-100 p-6">
              ...
              <div className="space-y-4">
                ...
                {report.certificateFile && (
                  <div className="border-t border-gray-100 pt-4">
                    <div className="flex items-center gap-2">
                      <FileText className="h-5 w-5 text-harvest-600" />
                      <a 
                        href={`${api.defaults.baseURL}/uploads/certificates/${report.certificateFile}`} 
                        target="_blank" 
                        rel="noopener noreferrer"
                        className="text-harvest-600 hover:text-harvest-700 underline"
                      >
                        View Certificate
                      </a>
                    </div>
              ...
              <div className="mt-6 flex gap-2">
                ...
                {report.certificateFile && (
                  <a 
                    href={`${api.defaults.baseURL}/uploads/certificates/${report.certificateFile}`}
                    download
                    className="flex-1 border border-gray-200 px-4 py-2 rounded-lg hover:bg-gray-50 transition-colors text-center"
                  >
                    Download
                  </a>
                )}
      ...
      {showUploadModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-md w-full max-h-[90vh] overflow-y-auto">
            <h2 className="text-xl font-semibold text-gray-800 mb-4">Submit Soil Report</h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                {formData.components.map((component, index) => (
                  <div key={index} className="flex items-center gap-2 mb-3">
                    <span className="w-1/3 text-gray-600">{component.name}</span>
                    <input
                      type="number"
                      step="0.01"
                      value={component.value}
                      onChange={(e) => handleComponentChange(index, 'value', e.target.value)}
                      className="w-1/3 px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-harvest-500"
                      required
                    />
                    <span className="w-1/4 text-gray-600">{component.unit}</span>
                  </div>
                ))}
              </div>
                  ...
                  >
                    {formData.certificateFile ? (
                      ...
                      <div>
                        ...
                        <p className="text-gray-600">Click to upload your certificate</p>
              ...
              <div className="flex justify-end gap-2 mt-6">
                ...
                <button 
                  type="submit"
                  className="px-4 py-2 bg-harvest-600 text-white rounded-lg hover:bg-harvest-700 transition-colors flex items-center gap-2"
                  disabled={submitting}
                >
                  {submitting ? (
                    <>
