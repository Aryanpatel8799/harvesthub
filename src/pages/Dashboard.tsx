...
const Dashboard = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="flex">
        <div className="flex-1 max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8">
          {/* Header section */}
          <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 pt-4 sm:pt-6">
            <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 sm:p-6">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-base sm:text-lg font-medium text-gray-700">Current Inventory</h3>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 sm:p-6">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-base sm:text-lg font-medium text-gray-700">Pending Orders</h3>
                <div className="bg-orange-100 rounded-full p-2">
                  <Truck className="h-5 w-5 text-orange-600" />
                </div>
              </div>
              <div className="flex flex-col">
                <div className="text-2xl sm:text-3xl font-bold text-gray-800">{stats.pendingOrders}</div>
                <div className="flex items-center mt-1 text-sm text-gray-500">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 sm:p-6">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-base sm:text-lg font-medium text-gray-700">Revenue (This Month)</h3>
          ...
          <div className="grid grid-cols-1 gap-4 sm:gap-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
              ...
              <div className="lg:col-span-1 space-y-4">
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
                  <h3 className="text-base sm:text-lg font-medium text-gray-700 mb-4">AI Suggestions</h3>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 sm:p-6">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4">
                <h3 className="text-base sm:text-lg font-medium text-gray-700">Recent Orders</h3>
                {orders.length > 0 && (
                  <button 
                    onClick={() => window.open('/orders-history', '_blank')}
                    className="mt-2 sm:mt-0 text-sm text-harvest-600 hover:text-harvest-700 flex items-center"
                  >
                    View All
              ...
              {loading ? (
                <div className="flex items-center justify-center py-6">
                  <Loader2 className="h-5 w-5 sm:h-6 sm:w-6 animate-spin text-harvest-600" />
                  <span className="ml-2 text-sm sm:text-base text-gray-600">Loading orders...</span>
                </div>
              ) : orders.length === 0 ? (
                ...
                <div className="space-y-3">
                  {(user?.type === 'farmer' ? orders.slice(0, 3) : orders.slice(0, 5)).map((order) => (
                    <div 
                      key={order._id} 
                      className={`border ${order.status === 'pending' ? 'border-yellow-200' : order.status === 'completed' ? 'border-green-200' : 'border-gray-200'} rounded-lg p-3 sm:p-4 hover:bg-gray-50 transition-colors`}
                    >
                      {user?.type === 'farmer' ? (
                          ...
                          <div className="flex items-center space-x-3">
                            ...
                            <div>
                              ...
                              <div className="flex items-center space-x-2 mt-1">
                                <span className="text-xs sm:text-sm text-gray-500">Order #{order._id.substring(0, 6)}</span>
                        ...
                        <div>
                          ...
                          <div className="flex flex-col sm:flex-row items-start space-y-4 sm:space-y-0 sm:space-x-4">
                            ...
                            <div className="flex flex-col sm:flex-row items-stretch sm:items-center space-y-2 sm:space-y-0 sm:space-x-2 w-full sm:w-auto">
                              <button 
                                onClick={() => window.open(`/order-details/${order._id}`, '_blank')}
                                className="flex-1 sm:flex-none px-4 py-2 bg-harvest-50 text-harvest-700 border border-harvest-200 rounded-md hover:bg-harvest-100 transition-colors text-sm font-medium"
                              >
                                Order Details
                              </button>
                              {order.status === 'accepted' && order.paymentStatus !== 'paid' && (
                                <button 
                                  ...
                                  className="flex-1 sm:flex-none px-4 py-2 bg-green-600 text-white rounded-md hover:bg-green-700 transition-colors text-sm font-medium"
                                >
                                  Complete Payment
                                </button>
                              )}
