...
const FarmerProfileForm = ({ initialData, onSubmit, onCancel }: FarmerProfileFormProps) => {
  ...
  const [previewImages, setPreviewImages] = useState<string[]>(initialData?.farmImages?.map(img => img.url) || []);
  const [previewVideos, setPreviewVideos] = useState<string[]>(initialData?.farmVideos?.map(vid => vid.url) || []);
  const [videoCaptions, setVideoCaptions] = useState<string[]>(initialData?.farmVideos?.map(vid => vid.caption || '') || []);
  const [imageCaptions, setImageCaptions] = useState<string[]>(initialData?.farmImages?.map(img => img.caption || '') || []);
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <Textarea
            ...
            onChange={handleInputChange}
            rows={4}
            placeholder="Tell consumers about your farming practices, experience, and specialties..."
          />
        </div>
        ...
        <div>
          <Label>Farming Videos (Max 3, 100MB each)</Label>
          ...
          <p className="mt-2 text-sm text-gray-500">
            Upload videos showing your farming practices, harvest process, or farm tour.
          </p>
        </div>
      </div>
